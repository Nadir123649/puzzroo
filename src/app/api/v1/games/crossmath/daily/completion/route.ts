...
function dailyChallengeIdFromDate(dateStr: string): string {
  ...
  if (parts.length === 3) {
    const [y, m, d] = parts
    return `daily-cross-math-${m.padStart(2, "0")}-${d.padStart(2, "0")}-${y.slice(-2)}`
  }
...
export const GET = withAuth(async (req: NextRequest, actor: Actor) => {
  if (!rateLimit(req, "crossmath-daily-completion", 30)) {
  ...
  try {
    ...
    const session = await CrossMathPlaySession.findOne(filter)
      .sort({ completedAt: -1 })
      .lean()
    ...
    console.error("[crossmath/daily/completion]", error)
