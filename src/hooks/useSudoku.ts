...
export function useSudoku() {
  ...
  const enterNumber = useCallback(
    (num: number) => {
      ...
      // Protection: If cell already has a value filled (and not in notes mode)
      if (!notesMode && cell.value !== null && cell.value !== undefined) {
        // Prevent re-filling or re-scoring if cell is already correct or matches input
        if (cell.isCorrect || cell.value === num) {
          return
        }
      }
  ...
  }, [selectedCell, gameState, updateScore, difficulty])
