...
function GameHeroContent({ name, image, imageLight, difficulties, gameSlug }: GameHeroProps) {
  ...
  const isChess = name.toLowerCase() === 'chess'
  const playUrl = isSudoku ? '/sudoku' : isCrossMath ? '/cross-math' : isNonogram ? '/nonogram' : isTangram ? '/tangram' : isChess ? '/chess/setup' : '#'
      ...
      <section className="w-full bg-white dark:bg-[#181A20] transition-colors duration-300 pt-12 md:pt-16 pb-12 relative">
        <div className="w-full px-[20px]">
          <div className="flex flex-col items-center gap-5 md:gap-6">
            ...
            {(isSudoku || isCrossMath || isNonogram || isTangram || isChess) ? (
              <button
            ...
            {/* OR Divider, Daily Challenge, and Past Puzzles (Hidden for Chess) */}
            {!isChess && (
              <>
                {/* OR Divider */}
                ...
                <Link prefetch={false} href={`/daily-challenge/${isSudoku ? 'sudoku' : isCrossMath ? 'cross-math' : isNonogram ? 'nonogram' : isTangram ? 'tangram' : 'sudoku'}`} className="w-full max-w-[382px]">
                ...
                <Link prefetch={false} href={`/past-puzzles/${isSudoku ? 'sudoku' : isCrossMath ? 'cross-math' : isNonogram ? 'nonogram' : isTangram ? 'tangram' : 'sudoku'}`} className="w-full max-w-[382px]">
